...
- [x] **M8-T01** Add `ideology_group`, `group_membership` schemas.
- [x] **M8-T02** `found_group` action: cult|party|union|club + doctrine. Doctrine generated by LLM (escalated `gpt-4o` call).
- [x] **M8-T03** `join_group` / `leave_group` actions; doctrine added to member prompt template.
- [x] **M8-T04** Weekly belief-update job: per agent, summarize the week's consolidated memories into a one-line belief; updates `traits.ideology_lean`.
- [x] **M8-T05** `/groups` page: list of all groups with doctrine excerpt, member count, founder, recent activity.
...
- [x] **M10-T01** Cost guard live with paging (Slack/email webhook). Soft 70% / hard 100%.
...
- [M10-T01] Budget hard-cap behavior blocks further provider calls and falls back to deterministic heuristics rather than pausing all simulation ticks, keeping the spectator product alive while spend is contained.
