...
function buildPrompt(agent: Agent, ctx: DecisionInput['context']): string {
  ...
  return [
    ...
    `FACTION: ${ctx.current_group_id ? `${ctx.current_group_name} (${ctx.current_group_kind}) doctrine: ${ctx.current_group_doctrine}` : 'no group'}${(ctx.candidate_groups?.length ?? 0) > 0 ? `; nearby groups: ${ctx.candidate_groups!.slice(0, 3).map((g) => `${g.id} ${g.name} ${g.kind}`).join('; ')}` : ''}`,
